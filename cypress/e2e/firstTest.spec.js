...
describe('First Test Suite', () =>{
    ...
    it('extract text values', () =>{
        ...
        cy.contains('Form Layouts').click()
            ...
            //5 invoke property
            cy.get('[id="exampleInputEmail1"]').type('elzatmozat@gmail.com')
            cy.get('[id="exampleInputEmail1"]').invoke('prop','value').should('contain','elzatmozat@gmail.com').then( property => {
                expect(property).to.include('elzatmozat@gmail.com')
            })
    ...
    it('checkBoxes', () =>{
        ...
        // recommand way: validate status of the selection of the checkbox
    ...
    it('Date Picker', () =>{
        ...
        it('List and dropdowns', ()=> {
            ...
            //2
            cy.get('nav nb-select').then( optionList =>{
                cy.wrap(optionList).click()
                cy.get('.options-list').each(( listItem, index) =>{
                    const itemText = listItem.text().trim()
                    cy.wrap(listItem).click()
                    cy.wrap(optionList).should('contain', itemText)
                        if(index<3){
                            cy.wrap(optionList).click()
                        }
        ...
        it('Web tables', () =>{
            ...
            cy.get('thead').find('tr').eq(2).then(TableRow =>{
                cy.wrap(TableRow).find('[placeholder="First Name"]').type('Elzat')
                cy.wrap(TableRow).find('[placeholder="Last Name"]').type('Azat')
                cy.wrap(TableRow).find('[class="nb-checkmark"]').click()

            } )

            cy.get('tbody tr').first().find('td').then( tableColumn =>{
                cy.wrap(tableColumn).eq(2).should('contain','Elzat')
                cy.wrap(tableColumn).eq(3).should('contain','Azat')
